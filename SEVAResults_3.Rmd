...
z <-  OverlapJunctionMatrices(junc.RPM = junc.RPM,
                              GenestoStudy = GenestoStudy)
MyRest <- z$Rest

## Tumor Samples
TumorSamp <- pheno %>% filter(classes=="Tumor") %>% dplyr::select(junctionSample) %>% unlist()
#pheno[which(pheno[,"classes"]=="Tumor"),"junctionSample"]#Tumor samples
NormSamp <- pheno %>% filter(classes=="Normal") %>% dplyr::select(junctionSample) %>% unlist()
#pheno[which(pheno[,"classes"]=="Normal"),"junctionSample"]#Normal samples
...
for( i in seq_along(GenestoStudy)){
  ...
  dist <- GSReg.kendall.tau.distance.Restricted.Sparse(
                            V = junc.RPM.NT[as.vector(GeneRestMat),],
                            RestMat = GeneRestMat)
  ...
  lines(x[NormSamp], y[NormSamp],   main=GenetoStudy,	type="p", col="blue", pch = 16)
   ...
   print( 
    ggplot(data.frame(Coordinate1=x,
                      Coordinate2=y,
                      Status=as.factor((setNames(pheno$classes,pheno$junctionSample))[names(x)])),
           aes(x=Coordinate1,
               y=Coordinate2,
               color=Status,
               shape=Status))+
...
ptm <- proc.time()
junctionPValue <- SEVA.meangeneFilter(junc.RPM = junc.RPM,
                              phenoVect=phenoVect,
                              minmeanloggeneexp= 3,
                              geneexpr=geneexpr)
cat("SEVA time:",proc.time() - ptm,"\n")

#Doing offline
#load("../Cache/SEVAJoe.rda")

#SEVA pvalue
SEVApvaluePure <- sapply(junctionPValue,FUN =  function(x) x$pvalue)
#FDR adjusted
SEVAGenesPure <- names(which(p.adjust(SEVApvaluePure,"BH")<0.01))
#Bonferonni correction (Not used)
#names(which(SEVApvaluePure<0.01/length(junctionPValue)))
...
#FDR
print(p.adjust(SEVApvaluePure[GenestoStudy],"BH"))

#### FDR
### VEGFC          DST        LAMA3         SDHA         TP63       RASIP1 
#### 2.597699e-01 2.880522e-10 1.540477e-05 8.544950e-01 1.732751e-09 9.513007e-07
...
cat("Genes survive the 0.01 threshold (FDR corrected)",
    names(which(p.adjust(SEVApvaluePure[GenestoStudy],"BH")<0.01)))
...
junctionPValueRandom <- SEVA.meangeneFilter(junc.RPM = junc.RPM.TCGA,
                                          phenoVect=phenoVect.TCGA,
                                          geneexpr=TCGA.RSEM,
                                          minmeanloggeneexp= 3,
...
# 
# 
# SEVA <- names(which(p.adjust(sapply(junctionPValue,function(x) x$pvalue),"BH")<0.01))
# 
# 
...
#                                         distparamPathways = MyRest  )
  ...
  junctionPValue <- SEVA.meangeneFilter(junc.RPM = junc.RPMext[,samplescur],
...
#  zscoresSEVA <- sapply(junctionPValue,FUN = function(x) max(abs(c(x$zscore,x$zscoreD12D1,x$zscoreD12D2))))
  ...
  transDiffSplice <- read.delim(
    paste("../Results/Simulation/DiffSplice/ResultsSim",
...
GenestoStudy <- c("ATAD3A","CD84","DHDDS","FCGR3A","IGSF8","MTX1","CCNL2","EXTL2","RC3H1" )

## Find the overlaps for all junctions.
z <-  OverlapJunctionMatrices(junc.RP =  junc.RPMext,
                              GenestoStudy = GenestoStudy)
MyRest <- z$Rest

## Tumor Samples
TumorSamp <- experimentSamplesTumors[[3]]#Tumor samples
NormSamp <- NormalLabels #Normal samples
### only Tumor and Normal samples
junc.RPM.NT <- cbind(junc.RPMext[,TumorSamp],junc.RPMext[,NormSamp])#only tumor and normal
...
y <- fit$points[,2]
  ...
  # lines(x[NormSamp],
  #       y[NormSamp], 
  #       main=paste("Simulated",GenetoStudy),	type="p", col="blue", pch = 16)
  
  print( 
    ggplot(data.frame(Coordinate1=x,
                      Coordinate2=y,
                      Status=as.factor(ifelse(grepl(x = names(x),
                                    pattern = "N$"), "Normal","Tumor"))),
           aes(x=Coordinate1,
               y=Coordinate2,
               color=Status,
               shape=Status))+
  ...
  pdf("../Results/Simulation/MDSplots.pdf",)
  ...
  ## heatmap of distances
  heatmap.2(x = dist[c(NormSamp,TumorSamp),c(NormSamp,TumorSamp)]/maxdist,
            main = paste("Simulated",GenetoStudy),
            Rowv = FALSE, Colv = FALSE, 
            colsep= length(NormSamp)+1,
            rowsep = length(NormSamp)+1,
            sepcolor = "white",
...
pdf("../Results/Simulation/MDSplotsSimulations.pdf")
for( i in seq_along(GenestoStudy)){
GenetoStudy <- GenestoStudy[i]
GeneRestMat <- MyRest[[GenetoStudy]]
...
Now, we analyze the second simulations:
```{r Second_Simulations,eval=TRUE,echo=TRUE,cache=FALSE,warning=FALSE,message=FALSE,fig.show='asis',fig.keep='all',dev=c('pdf', 'png'),fig.path="figure/fig",fig.width=7, fig.height=7}
library(dplyr)
library(tibble)
library(reshape2)
library(ggplot2)
...
### loading genes
gn <- genes(TxDb.Hsapiens.UCSC.hg19.knownGene)
gSymbol <- AnnotationDbi::select(org.Hs.eg.db,
                  keys=as.character(gn$gene_id),
                  columns=c('SYMBOL'),keytype='ENTREZID')
gn$SYMBOL <- gSymbol$SYMBOL

#loading the Truth
ASTruth <- (read.delim("../../NewSimData/Sim2/Data/Truth/AS_genes_list.txt",sep = "\t",header = F,stringsAsFactors = F))$V1 

load("../Results/Simulation/SecondTryJan13/isos2genesvect.rda")
isos2genesvectsimplified <- sapply(strsplit(isos2genesvect,split = '[|]'),FUN = function(x) x[1]) 


load("../Data/Data4Simulation/Sim2.Rdata")

phenotypes <- setNames(grepl(pattern = "tumor",
colnames(allJunctionsMat)),
colnames(allJunctionsMat))
## Scenarios

Scenarios <- vector(mode = "list",length = 4)
Scenarios[[1]] <- c(paste("normalSim_",1:20,sep = ""),
                   paste("tumorSim20_",1:20,sep = ""))
Scenarios[[2]] <- c(paste("normalSim_",1:20,sep = ""),
                   paste("tumorSim20_",1:10,sep = ""),
                   paste("tumorSim40_",11:20,sep = ""))
Scenarios[[3]] <- c(paste("normalSim_",1:20,sep = ""),
                   paste("tumorSim20_",1:10,sep = ""),
                   paste("tumorSim60_",11:20,sep = ""))
Scenarios[[4]] <- c(paste("normalSim_",1:20,sep = ""),
                   paste("tumorSim20_",1:10,sep = ""),
                   paste("tumorSim80_",11:20,sep = ""))


diffspliceFiles <- dir("../../PaperSuppl/Results/Simulation/DiffSplice/")

ebseqpvalueGenesAll <- vector(mode = "list",length = length(Scenarios))

DSEBseqAll <- ebseqpvalueGenesAll
SEVAAll <- ebseqpvalueGenesAll
DEEBSeqAll <- ebseqpvalueGenesAll
AllResults <- ebseqpvalueGenesAll
for( i in seq_along(Scenarios)){
  #current samples: Normals as nomral with a mixture of normal and cancerous as the cancer samples
  samplescur <- Scenarios[[i]]
  #phenotype
  phenotypescur <- setNames(grepl(pattern = "tumor",x=samplescur),samplescur)
  
  junctionPValue <- SEVA.meangeneFilter(junc.RPM=allJunctionsMat[,samplescur],
                                        phenoVect=as.factor(phenotypescur),
                                        geneexpr=allGeneExpressionMat[,samplescur],
                                        minmeanloggeneexp= 0)

  SEVA <- names(which(p.adjust(sapply(junctionPValue,function(x) x$pvalueTotal))<0.01))
  SEVAAll[[i]] <- SEVA
  
  zscoresSEVA <- sapply(junctionPValue,FUN = function(x) abs(x$zscore))
  

  DSEBseq_outcome <- findDSEBSEQ(allIsoformsMat[names(isos2genesvectsimplified)[which(isos2genesvectsimplified %in% names(junctionPValue))],samplescur],
                                 isos2genesvectsimplified,
                                 phenoVect =as.factor(phenotypescur) )
  
  DSEBseq <- DSEBseq_outcome$DSEBseq
  DSEBseqAll[[i]] <- DSEBseq
  
  DEEBSeq <- findDEGenes(geneexp = allGeneExpressionMat[names(junctionPValue),samplescur],
              
              phenoVect =as.factor(phenotypescur) ,pvaluecorrected = 0.05)
  
  DEEBSeqAll[[i]] <- DEEBSeq$DEGenes_EBesq
  
  if( i== 2){
    diffsplicefile <- read.delim("../../NewSimData/Sim2/ResultsSimN40/result/differential_transcription.txt",sep = "\t")
    
  }else if(i==3){
    diffsplicefile <- read.delim("../../NewSimData/Sim2/ResultsSimN60/result/differential_transcription.txt",sep = "\t")
  }else if(i==4){
    diffsplicefile <- read.delim("../../NewSimData/Sim2/ResultsSimN80/result/differential_transcription.txt",sep = "\t")
  ...
  if(i == 1){
    ...
    junctionsDiffSplice <- GRanges(seqnames = Rle(diffsplicefile[,"chromosome"]), 
                                   ranges = IRanges(start = as.numeric(diffsplicefile[,"position_start"]), 
                                                    end = as.numeric(diffsplicefile[,"position_end"])))
    
    DiffSplicehits <- findOverlaps(junctionsDiffSplice,gn)
    DiffSplicegenes <- unique(gn$SYMBOL[subjectHits(DiffSplicehits)])
  }
  AllResults[[i]] <- inner_join(data.frame(Gene = names(junctionPValue)) %>% 
                                  mutate(DS = Gene %in% ASTruth, 
                                         SEVA = Gene %in% SEVA,
                                         EBSeq = Gene %in% DEEBSeq$DEGenes_EBesq,
                                         DE = Gene %in% DEEBSeq$DEGenes_EBesq,
                                         DiffSplice = Gene %in% DiffSplicegenes ),
                                data.frame(LogMeanExp = 
                                             (allGeneExpressionMat %>% rowMeans()+1)%>%
                                             log2) %>% 
                                  rownames_to_column("Gene" ) %>% 
                                  transmute(Gene=Gene,
                                            ExpressionLevel = cut(LogMeanExp,c(-Inf,4,9,Inf))),by="Gene") %>% 
    
    inner_join(y=data.frame(IsoformNum =
                              tapply(INDEX =  isos2genesvectsimplified,
                                     X =names(isos2genesvectsimplified), 
                                     FUN = length) ) %>% 
                 rownames_to_column("Gene") %>% 
                 filter(Gene!="?"),by = "Gene")
...
names(AllResults) <- paste(c(20,40,60,80))
NewSimulations <- 
  sapply(AllResults[2:4],function(y){  
    x <- y %>% filter(IsoformNum>1);
    setNames(c(x %>% filter(SEVA==T)  %>% summarise(mean(DS)) %>% unlist ,
               x %>% filter(DiffSplice==T)  %>% summarise(mean(DS))%>% unlist,
               x %>% filter(EBSeq==T)  %>% summarise(mean(DS))%>% unlist),c("SEVA","DiffSplice","EBSeq"));
    }) 
NewSimulationsForPlot <- melt(NewSimulations)
colnames(NewSimulationsForPlot) <- c("Method","Perturbed of Isoform","Precision (at FDR 1%)")

NewSimulationsForPlot$`Perturbed of Isoform`<- 
  as.numeric(NewSimulationsForPlot$`Perturbed of Isoform`)

print(ggplot( NewSimulationsForPlot,
      aes(x=`Perturbed of Isoform`,
          y=`Precision (at FDR 1%)`,
          shape=Method ,
          color=Method))+
geom_point()+geom_line())

pdf("../../NewSimData/Sim2/SecondSimulations.pdf")
print(ggplot( NewSimulationsForPlot,
        aes(x=`Perturbed of Isoform`,
            y=`Precision (at FDR 1%)`,
            shape=Method ,
...
NewSimulationsLevels <- bind_rows(sapply(names(AllResults)[2:4],
                                         function(x) AllResults[[x]] %>% 
                                           mutate(`Perturbed of Isoform`=x),simplify = F)) %>% 
  filter(IsoformNum>1)

NewSimulationsLevelsPlot <- 
  rbind(NewSimulationsLevels %>% group_by(ExpressionLevel,`Perturbed of Isoform`)%>%filter(EBSeq==T) %>% summarise(mean(DS)) %>% as.data.frame() %>% mutate(Method="EBSeq"),
        NewSimulationsLevels %>% group_by(ExpressionLevel,`Perturbed of Isoform`)%>%filter(SEVA==T) %>% summarise(mean(DS)) %>% as.data.frame()%>% mutate(Method="SEVA"),
        NewSimulationsLevels %>% group_by(ExpressionLevel,`Perturbed of Isoform`)%>%filter(DiffSplice==T) %>% summarise(mean(DS)) %>% as.data.frame()%>% mutate(Method="DiffSplice")) 
colnames(NewSimulationsLevelsPlot)[3] <- "Precision (at FDR 1%)"  

print( ggplot(NewSimulationsLevelsPlot,
              aes(y=`Precision (at FDR 1%)`,
                  x=`Perturbed of Isoform`,
                  shape=Method,
                  group =Method,
                  color=Method))+
         geom_point()+
         geom_line()+
         facet_wrap(~ExpressionLevel))

pdf("../../NewSimData/Sim2/SecondSimulationsForDifferentExpressionValue.pdf")
print( ggplot(NewSimulationsLevelsPlot,
              aes(y=`Precision (at FDR 1%)`,
                  x=`Perturbed of Isoform`,
                  shape=Method,
...
NewSimulationsIsoformNum <- 
  sapply(2:4,function(y){
     AllResults[[y]] %>% filter(IsoformNum>1) %>%mutate(`Isoform Num` =cut(IsoformNum,breaks = c(2,3,4,5,Inf),include.lowest = T,right = F,labels = c("2","3","4","5+"))) %>% group_by(`Isoform Num`)%>%summarise(count=n(),SEVA=sum(DS*SEVA)/sum(SEVA),DiffSplice=sum(DS*DiffSplice)/sum(DiffSplice),EBSeq=sum(DS*EBSeq)/sum(EBSeq)) %>% as.data.frame() %>% transmute(`Isoform Num`=paste("#Isoform =",`Isoform Num`),SEVA=SEVA,DiffSplice=DiffSplice,EBSeq=    EBSeq,`Perturbed of Isoform`=names(AllResults)[y])
    },simplify = F)  %>% bind_rows()
NewSimulationsIsoformNumForPlot <- melt(NewSimulationsIsoformNum,
                                        variable.name = "Method",
                                        id.vars = c("Isoform Num", "Perturbed of Isoform"),
                                        value.name = "Precision (at FDR 1%)")

print( ggplot(NewSimulationsIsoformNumForPlot,
              aes(y=`Precision (at FDR 1%)`,
                  x=`Perturbed of Isoform`,
                  shape=Method,
                  group =Method,
         ...
         geom_point()+
         geom_line()+
         facet_wrap(~`Isoform Num`))

pdf("../../NewSimData/Sim2/SecondSimulationsIsoformCount.pdf")
print( ggplot(NewSimulationsIsoformNumForPlot,
              aes(y=`Precision (at FDR 1%)`,
                  x=`Perturbed of Isoform`,
                  shape=Method,
...
MyRecalls <- rbind(sapply(recallall,FUN = function(x) setNames(x,c(10,15,20,25))) %>% as.data.frame() %>% rownames_to_column("Number of tumor samples with event") %>% mutate(`Gene Expression`="Differentially spliced genes"),sapply(recallDEall,FUN = function(x) setNames(x,c(10,15,20,25))) %>% as.data.frame() %>% rownames_to_column("Number of tumor samples with event") %>% mutate(`Gene Expression`="Differentially spliced and expressed genes")) %>% melt(variable.name="Method",value.name="Recall",id.vars=c("Number of tumor samples with event", "Gene Expression"))

MyPrecision <- rbind(sapply(precisionall,FUN = function(x) setNames(x,c(10,15,20,25))) %>% as.data.frame() %>% rownames_to_column("Number of tumor samples with event") %>% mutate(`Gene Expression`="Differentially spliced genes"),sapply(precisionDEall,FUN = function(x) setNames(x,c(10,15,20,25))) %>% as.data.frame() %>% rownames_to_column("Number of tumor samples with event") %>% mutate(`Gene Expression`="Differentially spliced and expressed genes")) %>% melt(variable.name="Method",value.name="Precision",id.vars=c("Number of tumor samples with event", "Gene Expression"))

pFig2 <- ggplot(inner_join(MyPrecision,MyRecalls,by=c("Number of tumor samples with event","Gene Expression","Method"))%>% melt(id.vars=c("Number of tumor samples with event", "Gene Expression","Method")),
       aes(x=`Number of tumor samples with event`,y=value,linetype=`Gene Expression`,color=Method,shape=Method,group=interaction(`Gene Expression`,Method)))+geom_point()+geom_line()+facet_wrap(~variable)+theme(axis.title.y=element_blank(),legend.position = "bottom",legend.title = element_blank())+guides(color=guide_legend(ncol=2,nrow=2,byrow=TRUE),linetype=guide_legend(ncol=1,nrow=2,byrow=TRUE))+scale_linetype_manual(values=c("dashed","solid"))
save(list = "pFig2",file = "../Cache/Fig2.rda")

print(pFig2)
pdf("../Results/Simulation/Figure2NoRMATS.pdf")
print(pFig2)
dev.off()
### Rerun the file "../Scripts/Simulation/GeneraterMatsAcc.R"
```
